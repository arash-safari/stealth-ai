# src/sim_status_llm.py
# Simple simulator for the STATUS flow.
# Run: python3 -m src.sim_status_llm
import os, builtins
...
from .status_agent import run_status_flow
...
persona = {
    ...
    "starter": "Hi, what’s the status of my appointment?",
}

def persona_system_prompt() -> str:
    return (
        f"You are {persona['name']}, a {persona['personality']} homeowner.\n"
        "You are chatting with a plumbing company's assistant about APPOINTMENT STATUS.\n"
        "- Answer VERY concisely with JUST the answer.\n"
        "- If asked for appointment number/ID: reply with it exactly.\n"
        "- If the assistant tells you your status and scheduled window, you don't need to say anything more.\n"
...
def run_with_persona(initial_user_text: str):
    ...
    def is_questionish(agent_text: str) -> bool:
        ...
        if t.endswith("?"):
            return True
        if "appointment number" in t or "appointment id" in t or "id" in t:
            return True
        return False
    ...
    try:
        ...
        run_status_flow(initial_user_text)
    finally:
...
if __name__ == "__main__":
    ...
    # Be forgiving in tests: run STATUS flow even if classifier misfires
    if intent.get("intent_code") != "STATUS":
        print("Classifier did not return STATUS; running STATUS flow anyway for test.\n")
